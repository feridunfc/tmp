...
def validate_ohlcv(df: pd.DataFrame, *, schema: OhlcvSchema|None=None, raise_errors: bool=True) -> Tuple[pd.DataFrame, Dict[str,Any]]:
    ...
    if missing:
        report["ok"] = False; report["missing"] = missing
        if raise_errors: raise ValueError(f"Missing required columns: {missing}")
