...
def render(ctx):
    st.subheader("🔎 Hiperparametre Optimizasyonu (HPO)")

    if optuna is None:
        st.warning("Optuna bulunamadı. `pip install optuna` ile kurabilirsiniz.")
        return
    ...
    with st.form("hpo_form"):
        ...
        direction = st.selectbox("Yön", ["maximize", "minimize"], index=0, key="hpo_dir")
        n_trials = st.number_input("Deneme Sayısı", 1, 500, 20, 1, key="hpo_ntrials")
        run_btn = st.form_submit_button("HPO Başlat", type="primary", width='stretch')
    ...
    st.success("HPO tamamlandı.")
    st.json({"best_value": study.best_value, "best_params": study.best_params})

    vals = [t.value for t in study.trials if t.value is not None]
    plot_histogram(vals, title=f"{metric} dağılımı", bins=40, key="hpo_hist")
